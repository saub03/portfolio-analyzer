...
class AIAnalyzer:
    ...
    def __init__(self, api_key):
        ...
        logger.info("genai.Client 초기화 완료 (모델: gemini-3.5-flash)")
    ...
    def generate_ai_report(self, calendar_data, news_data, user_data):
        """
        수집된 캘린더, 뉴스, 포트폴리오 데이터를 바탕으로 종합 AI 리포트를 작성
        """
        logger.info("AI 종합 리포트 생성 요청 중...")
        
        prompt = f""" 
        당신은 거시 경제 분석과 금융 공학에 능통한 수석 프라이빗 뱅커(PB)입니다. 
        ...
        - 'profit_loss_summary' 데이터를 활용하여 각 자산군별 투자 원금 대비 현재 가치의 증감액과 수익률을 명확히 분석하세요.
        - 샤프 지수, 변동성, 베타, MDD, 수익률 등의 통계 데이터를 활용하여 포트폴리오 성과와 효율성을 분석하세요.
        - 2.1 및 2.2 항목의 표는 가독성을 위해 명확한 열(Column)을 설정하세요. (예: 자산명, 수익률, 샤프지수, 변동성, MDD 등)
        ...
        [리포트 목차 및 세부 작성 가이드 (반드시 아래 마크다운 구조와 표 형식을 100% 동일하게 따를 것)]
        ### 1. 시장 시황 및 주요 경제 지표 요약
        (거시 경제, 증시, 채권 시장 요약 후 중요한 경제 지표/일정을 글머리기호(*)로 나열)
        ...
        **종합 분석:**
        (1. 주식군, 2. 채권군, 3. 가상자산군 등으로 번호를 매겨 핵심 분석)

        #### 2.3. 상관관계 분석
        (자산 간의 상관관계에 대한 분석을 글머리기호(*)로 작성)

        ### 3. 투자자 성향 및 재무 상태 진단
        *   **투자자 성향 및 목표:**
        *   **재무 체력 진단:** (부채 규모, 비상 자금, 월 저축 여력을 명시)
        *   **종합 진단:** 

        ### 4. 리밸런싱 조언
        #### 4.1. 자산군별 리밸런싱 조언
        (1. 주식군: [현재 XX% -> 목표 YY% (Z% 비중 확대/축소)] 형식으로 작성)
        #### 4.2. 특정 자산 추가 매수, 매도 조언
        *   **추가 매수 (Buy):** 
        *   **부분 매도 및 슬림화 (Sell/Trim):** 
        #### 4.3. 시나리오 분석 (수집된 뉴스의 거시경제 이벤트 기반)
        (시나리오 A, B를 나누고 각각에 대해 '*전제:*', '*영향:*', '*대응:*' 항목을 작성)
        #### 4.4. 투자 논리 재검토 및 시장 대응
        (각 자산군의 기존 투자 논리가 여전히 유효한지 '*검토:*' 형식으로 작성)
        """
